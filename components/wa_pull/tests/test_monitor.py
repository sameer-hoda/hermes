...
def test_trigger_detection_user_asked_group_with_name():
    """user_asked triggers in a group when @LID mention + question mark."""
    m = PulseMonitor()
    m._own_lid_num = "123456789"
    msgs = [
        {"chat_jid": "grp@g.us", "chat_name": "Test", "content": "@123456789 can you review this?", "is_from_me": False, "sender": "Alice"},
    ]
    trigger, trigger_msgs = m._detect_trigger("grp@g.us", msgs)
    # @LID takes priority over user_asked, so this should be user_tagged or user_asked
    assert trigger in ("user_tagged", "user_asked")
